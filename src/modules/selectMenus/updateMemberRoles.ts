...
export const updateMemberRoles = async (client: any) => {
  client.on("interactionCreate", async (interaction: any) => {
    if (interaction.customId !== "select-menu-roles") {
      return;
    }

    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const clientRole = interaction.guild.members.cache.get(client.user.id).roles
      .highest;
