...
export const onGuildUpdate = async (
  ...
  const guildEmbed = new MessageEmbed()
    ...
    .addField("Old Guild Description", `| ${oldGuild.description}`, false)
    .addField("New Guild Description", `| ${newGuild.description}`, false)
    .setThumbnail(newGuild.iconURL())
    .setTimestamp();
  logHandler.log(
    "info",
    `${newGuild.name} has been updated. Guild ID: ${newGuild.id}`
  );
